...
syntax [varlist(numeric fv default=none)], [ci(real 95) constant delta  xvar(varname) ///
                                            graph(str) rarea ci_off pci tol(real 0.1)  addgraph(str) ]
     ...
     tempname xm  
	   ...
	   *** here is the graph
	   if "`delta'"=="" local ytl "{&beta}(z)"
	   	   ...
	   	   if "`ci_off'"=="" {
		       if "`addgraph'"!="" {
				   twoway (`gtype' `xm'2 `xm'3 `vcx', sort) (line `xm'1 `vcx', sort) (`addgraph'), ///
				   ...
				   twoway (`gtype' `xm'2 `xm'3 `vcx', sort) (line `xm'1 `vcx', sort) , ///
				   xtitle("`lbl'") ytitle(`ytl') legend(order(1 "`ci'% `p' Confidence Interval")) ///
				   title("Varying Coefficients of `tdelta'`x'") name(`graph'`cnt', replace)
			   }
		   }
		   if "`ci_off'"=="ci_off" {
		   	   if "`addgraph'"!="" {
				   twoway line (`xm'1 `vcx', sort) (`addgraph'), ///
				   xtitle("`lbl'") ytitle(`ytl') legend(off) ///
				   title("Varying Coefficients of `tdelta'`x'") name(`graph'`cnt', replace)
			   }
			   else {
				   twoway (line `xm'1 `vcx', sort) , ///
				   xtitle("`lbl'") ytitle(`ytl') legend(order(1 "`ci'% `p' Confidence Interval")) ///
				   title("Varying Coefficients of `tdelta'`x'") name(`graph'`cnt', replace)
			   }
		   }
